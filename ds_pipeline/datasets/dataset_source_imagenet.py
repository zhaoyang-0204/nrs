...
from nrs.ds_pipeline.autoaugment import autoaugment
from nrs.ds_pipeline.datasets import dataset_source
...
def preprocess_for_train(image_bytes: tf.Tensor,
  ...
  image = _decode_and_random_crop(image, image_size)
...
class Imagenet(dataset_source.DatasetSource):
  ...
  def _pp(self, path : str) -> Dict[str, tf.Tensor]:
    ...
    return dict(
        image=path,
        label=tf.where(
            tf.strings.split(path, '/')[-2] == self.class_names
        )[0][0],
    )
  ...
  def get_train(self, use_augmentations: bool) -> tf.data.Dataset:
    ...
    ds = ds.map(self._pp, tf.data.experimental.AUTOTUNE)
  ...
  def get_test(self) -> tf.data.Dataset:
    """Returns test set."""
    ds = self._test_ds.map(self._pp, tf.data.experimental.AUTOTUNE)
    ds = ds.map(
  ...
  def decode_example(self, example: Dict[str, tf.Tensor],
                     use_augmentations: bool = True) -> Dict[str, tf.Tensor]:
